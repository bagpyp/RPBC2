...
def fromECM(run=True,ecm=True,drive='E',stid = stid):    
    ...
    if run:
        invns = []
        for file in glob(fr'{drive}:\ECM\Polling\{stid}\OUT\Inventory*'):    
        ...
        v_names = {v.attrib['vend_code']:v.attrib['vend_name'] for v in \
                ET.parse(fr'{drive}:\ECM\Polling\{stid}\OUT\Vendor.xml')\
                    .getroot().findall('VENDORS/VENDOR')}
        df['brand'] = df['vend_code'].map(v_names)
        
        # DCSes (or categories) a df itself
        cats = pd.DataFrame([cat.attrib for cat in \
                ET.parse(fr'{drive}:\ECM\Polling\{stid}\OUT\DCS.xml')\
                    .getroot().findall('DCSS/DCS')])\
            .replace('',nan).dropna(how='all',axis=1)\
            [['dcs_code','d_name','c_name','s_name']]\
            .dropna(thresh=2)
        ...
        # first pickle
        df.to_pickle('rawFromECM.pkl')
        
        # naming map for df
        names = {'style_sid':'ssid',
        'item_sid':'isid',
        'alu':'sku',
        'upc':'UPC2',
        'local_upc':'UPC',
        ...
        return df
    if not run:
        return pd.read_pickle('fromECM.pkl')