...
def configureOptions(df):
    ...
    optionDf.to_pickle('optionDf.pkl')
    return optionDf
...
def mediate(g):
    if len(g)>1:
        ...
        # lift all else p_
        for p in ['p_name', 'p_sku', 'p_categories', 'p_description',
        'p_is_visible', 'p_date_created', 'p_date_modified', 'p_id',
        'description']:
            if g1[p].count():
...
def archiveMedia(df):
    ...
    # base images
    bases = [b.split('\\')[-1].split('.')[0] \
            for b in glob('images\\base\\*.jpeg')]
    base = df[df.sku.str[:2]!='1-']\
    [['sku']+[f'image_{i}' for i in range(5)]]\
    ...
    # variant images
    variants = [b.split('\\')[-1].split('.')[0] \
            for b in glob('images\\variant\\*.jpeg')]
    ...
    #picklin' pics
    media = pd.read_pickle('media.pkl')
    media_now = \
        df[['sku','v_image_url','description']+[f'image_{i}' for i in range(5)]]\
        .set_index('sku')
    media.update(media_now)
    media = pd.concat([media,media_now[\
        ~media_now.index.isin(media.index)]])
    media.to_pickle('media.pkl')

def fileDf():
    v = [g.split('\\')[-1].split('.')[0] \
                              for g in glob('images\\variant\\*')]
    b = pd.DataFrame([g.split('\\')[-1].split('.')[0] \
                              for g in glob('images\\base\\*')])
    b = b[b[0].str.contains('_')][0].str.split('_',expand=True)[0]\
        .value_counts().to_dict()
    files = pd.DataFrame(index=list(b.keys()) + v,\
                         columns=['v_image_url']\
